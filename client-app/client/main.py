...
class ModernChatApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Secure StegoChat Client")
        self.geometry("900x600")
        
        # Sunucu IP Sorma
        self.server_ip = ctk.CTkInputDialog(text="Sunucu IP Adresi (Örn: 192.168.1.35):", title="Bağlantı Ayarı").get_input()
        if not self.server_ip: self.server_ip = "127.0.0.1"
    ...
    def setup_socket_events(self):
        @self.sio.on('connect')
        def on_connect(): print("Bağlantı başarılı!")

        @self.sio.on('user_list_update')
        def on_user_list(data): self.update_user_list_ui(data)
    ...
    def do_register(self):
        if not self.entry_user.get() or not self.entry_pass.get() or not self.selected_image_path: return
        res = self.api.register(self.entry_user.get(), self.entry_pass.get(), self.selected_image_path)
    ...
    def show_chat_ui(self):
        ...
        self.msg_entry = ctk.CTkEntry(input_frame)
    ...
    def update_user_list_ui(self, data):
        ...
        for u in data['users']:
            ...
            online = u in data['online']
            ctk.CTkButton(self.user_scroll, text=f"{u}\n({'Online' if online else 'Offline'})", fg_color="#27AE60" if online else "gray", command=lambda x=u: self.select_recipient(x)).pack(pady=5, fill="x")
    ...
    def send_message(self):
        ...
        if not msg or not self.selected_recipient: return
        self.sio.emit('send_message', {'sender': self.username, 'receiver': self.selected_recipient, 'message': self.crypto.encrypt(msg)})
        self.chat_display.configure(state="normal")
